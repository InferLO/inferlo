...
from inferlo import DiscreteDomain, DiscreteFactor, GenericGraphModel
from inferlo.generic import inference as inf
from inferlo.testing import (
...
def test_all_tree_30():
    ...
    assert np.allclose(inf.belief_propagation(model).log_pf, true_log_pf, atol=1e-5)
    assert np.allclose(inf.iterative_join_graph_propagation(model), true_log_pf, atol=1e-5)
    random.seed(0)
    assert np.allclose(inf.mini_bucket_elimination(model), true_log_pf, atol=1e-5)
    assert np.allclose(inf.bucket_elimination(model), true_log_pf, atol=1e-5)
    assert np.allclose(inf.weighted_mini_bucket_elimination(model), true_log_pf, atol=0.5)
    assert np.allclose(inf.bucket_renormalization(model), true_log_pf, atol=1e-5)
...
def test_all_grid_6x6():
    ...
    assert_results_close(
        inf.belief_propagation(model), true_inference, log_pf_tol=0.1, mp_mse_tol=1e-3
    )
    assert np.allclose(inf.iterative_join_graph_propagation(model), true_log_pf, atol=1e-5)
    assert np.allclose(inf.mini_bucket_elimination(model), true_log_pf, atol=10.0)
    assert np.allclose(inf.bucket_elimination(model), true_log_pf, atol=1e-5)
    assert np.allclose(inf.weighted_mini_bucket_elimination(model), true_log_pf, atol=10.0)
...
def test_all_clique_6():
    ...
    assert np.allclose(inf.belief_propagation(model).log_pf, true_log_pf, atol=0.1)
    assert np.allclose(inf.iterative_join_graph_propagation(model), true_log_pf, atol=1e-4)
    assert np.allclose(inf.mini_bucket_elimination(model), true_log_pf, atol=1e-5)
    assert np.allclose(inf.bucket_elimination(model), true_log_pf, atol=1e-5)
    assert np.allclose(inf.weighted_mini_bucket_elimination(model), true_log_pf, atol=1e-5)
    assert np.allclose(inf.bucket_renormalization(model), true_log_pf, atol=1e-5)
...
def test_wmbe_grid_9x9():
    ...
    assert np.allclose(inf.weighted_mini_bucket_elimination(model), true_log_pf, atol=4.0)


def test_bucket_renormalization_grid_9x9():
    model = grid_potts_model(9, 9, al_size=2, seed=0)
    true_log_pf = model.infer(algorithm="path_dp").log_pf
    assert np.allclose(inf.bucket_renormalization(model), true_log_pf, atol=4.0)
...
def test_get_marginals():
    ...
    be_result_1 = inf.get_marginals(model, inf.bucket_elimination, skip_last=False)
    be_result_2 = inf.get_marginals(model, inf.bucket_elimination, skip_last=True)
...
def test_get_marginals_with_different_algos():
    ...
    model.add_factor(
        DiscreteFactor(model, [0, 1, 2], [[[0.999, 0.001], [0.9, 0.1]], [[0.95, 0.05], [0.1, 0.9]]])
    )
    model.add_factor(
        DiscreteFactor(
            model, [0, 1, 3], [[[0.999, 0.001], [0.8, 0.2]], [[0.95, 0.05], [0.05, 0.95]]]
        )
    )
    model.add_factor(
        DiscreteFactor(
            model, [0, 1, 4], [[[0.999, 0.001], [0.7, 0.3]], [[0.95, 0.05], [0.025, 0.975]]]
        )
    )
    algos = [
