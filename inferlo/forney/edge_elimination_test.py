...
import inferlo
from inferlo import PairWiseFiniteModel, DiscreteDomain, OldDiscreteFactor
...
def test_convolve_factor():
    ...
    factor1 = inferlo.OldDiscreteFactor.from_factor(factor1)
...
def test_convolve_two_factors():
    ...
    factor1 = inferlo.OldDiscreteFactor.from_values(model, [0, 1], v1)
    factor2 = inferlo.OldDiscreteFactor.from_values(model, [1, 2], v2)
...
def test_convolve_two_factors_leaf():
    ...
    factor1 = inferlo.OldDiscreteFactor.from_values(model, [0], np.array([1, 2]))
    factor2 = inferlo.OldDiscreteFactor.from_values(model, [0, 1], np.array([[3, 4], [5, 6]]))
...
def test_infer_small_line():
    ...
    model *= inferlo.OldDiscreteFactor.from_values(model, [0], f0)
    model *= inferlo.OldDiscreteFactor.from_values(model, [0, 1], f01)
    model *= inferlo.OldDiscreteFactor.from_values(model, [1, 2], f12)
    model *= inferlo.OldDiscreteFactor.from_values(model, [2], f2)
    model.build()
...
def test_infer_clique_3vars():
    ...
    model *= inferlo.OldDiscreteFactor.from_values(model, [0, 1], f01)
    model *= inferlo.OldDiscreteFactor.from_values(model, [1, 2], f12)
    model *= inferlo.OldDiscreteFactor.from_values(model, [2, 0], f20)
    model.build()
...
def test_infer_one_variable():
    ...
    model.add_factor(OldDiscreteFactor.from_values(model, [0], [1, 2]))
    model.add_factor(OldDiscreteFactor.from_values(model, [0], [1, 2]))
    model.build()
