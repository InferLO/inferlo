...
class DiscreteFactor(Factor):
    ...
    def from_factor(factor: Factor) -> DiscreteFactor:
        ...
        # TODO: use from_flat_values?
        new_factor = DiscreteFactor.from_values(factor.model, factor.var_idx,
    ...
    def from_values(model: GraphModel,
        ...
        return DiscreteFactor(model, var_idx, log_values)
    ...
    def from_flat_values(model: GraphModel,
        ...
        return DiscreteFactor.from_values(model, var_idx, values)
    ...
    def restrict(self, variable_id, fixed_value) -> DiscreteFactor:
        ...
        return DiscreteFactor(self.model, new_var_idx, new_log_values)

    def marginalize(self, var_idx: List[int] = None, operator="sum", **kwargs) -> DiscreteFactor:
        ...
        return DiscreteFactor(self.model, new_vars_idx, new_log_values)

    def copy(self):
        return DiscreteFactor(self.model, self.var_idx, self.log_values)

    def clone(self, new_model: GraphModel):
        return DiscreteFactor(new_model, copy.copy(self.var_idx), self.log_values)
